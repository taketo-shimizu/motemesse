...
async def generate_reply(request: ReplyRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if conversation and len(conversation) > 0:
            ...
            # 最初のレコードのfemale_messageがNoneまたは空文字列の場合、すでに男性から挨拶済み
            if not first_message.female_message or first_message.female_message.strip() == "":
        ...
        greeting_constraint = ""
        if already_greeted:
            greeting_constraint = "\n\n**重要な制約**: あなたはすでに相手に初回挨拶を送信済みです。返信には「初めまして」「はじめまして」「よろしくお願いします」などの挨拶表現を含めないでください。会話を継続する内容のみを生成してください。"
...
## 最重要ルール ##
### 話題は「食」または「場所」限定
- 例: カフェ/ご飯/ランチ/ディナー/スイーツ/居酒屋/コーヒー、最寄りエリア/駅近/帰り道/職場や自宅の周辺など。  
- 他ジャンル（音楽・映画・仕事・趣味等）は出さない。必要な場合は食/場所に橋渡しして話す。
...
6. **固有名詞の処理**: 具体的な店名が必要な場合は「<<< 自分で入力 >>>」形式で表記
## 安全ガイドライン
- 18歳未満への対応禁止
